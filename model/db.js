...
exports.init = function(){
    ...
    return connection.query("CREATE database if not exists test ").
		...
		then(connection.query("CREATE TABLE IF NOT EXISTS VIAJES (ID INT PRIMARY KEY AUTO_INCREMENT, CHOFER VARCHAR(50), LAT DOUBLE, LON DOUBLE, DIR VARCHAR(200), fecha TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")).
		then(connection.query("CREATE VIEW VIAJES_PENDIENTES AS SELECT ID, CHOFER, LAT, LON, FECHA, DIR FROM VIAJES WHERE CHOFER IS NULL AND FECHA > (CURRENT_TIMESTAMP + INTERVAL 2 MINUTE)")).
